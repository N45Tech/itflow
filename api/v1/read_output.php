...
if ($sql && mysqli_num_rows($sql) > 0) {
    ...
    logApp('API', 'Error', 'Read query returned no accessible resource on API call to '
        . escapeSql(parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH) ?: '')
        . ' containing ' . count($_GET) . ' GET variables via API key '
        . escapeSql($api_key_name) . ' from IP ' . escapeSql(getIP())
        . ' with agent ' . escapeSql(getUserAgent()));
