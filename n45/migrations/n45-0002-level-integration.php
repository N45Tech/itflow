...
/*
 * N45 migration n45-0002-level-integration (legacy marker 2.6.9)
...
"CREATE TABLE IF NOT EXISTS `level_group_mappings` (
...
"CREATE TABLE IF NOT EXISTS `level_asset_links` (
...
"CREATE TABLE IF NOT EXISTS `level_alert_links` (
...
"CREATE TABLE IF NOT EXISTS `level_webhook_events` (
...
"ALTER TABLE `level_webhook_events`
    ADD COLUMN IF NOT EXISTS `level_webhook_processing_at` datetime DEFAULT NULL AFTER `level_webhook_last_received_at`",
];

foreach ($level_migration_queries as $level_migration_query) {
    if (!mysqli_query($mysqli, $level_migration_query)) {
        throw new RuntimeException('Could not apply the N45 Level schema: ' . mysqli_error($mysqli));
    }
}
