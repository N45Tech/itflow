    ...
    string $strategy, array $metadata = []): void
{
    ...
    $type_sql = automationDbEscape($entity_type);
    $external_sql = automationDbEscape($external_id);
    $name_sql = automationDbEscape(automationLimitText($external_name, 255));
    $strategy_sql = automationDbEscape(automationLimitText($strategy, 40));
    ...
    automationDbQuery("INSERT INTO automation_entity_mappings SET
        automation_mapping_source = '$source_sql', automation_mapping_entity_type = '$type_sql',
        automation_mapping_external_id = '$external_sql', automation_mapping_external_name = '$name_sql',
        automation_mapping_client_id = $client_id, automation_mapping_location_id = $location_id,
        automation_mapping_asset_id = $asset_id, automation_mapping_domain_id = $domain_id,
        automation_mapping_strategy = '$strategy_sql',
        automation_mapping_metadata = '$metadata_sql', automation_mapping_last_seen_at = NOW()
        ON DUPLICATE KEY UPDATE automation_mapping_external_name = VALUES(automation_mapping_external_name),
        automation_mapping_client_id = VALUES(automation_mapping_client_id),
        automation_mapping_location_id = VALUES(automation_mapping_location_id),
        automation_mapping_asset_id = VALUES(automation_mapping_asset_id),
        automation_mapping_domain_id = VALUES(automation_mapping_domain_id),
        automation_mapping_strategy = VALUES(automation_mapping_strategy),
        automation_mapping_metadata = VALUES(automation_mapping_metadata),
        automation_mapping_last_seen_at = NOW(), automation_mapping_deleted_at = NULL",
...
function automationCreateIncidentTicket(array $event, array $resolved): array
{
    ...
    $priority = ucfirst(strtolower(automationLimitText($event['severity'] ?? 'low', 20)));
...
function automationAddIncidentReply(int $ticket_id, int $client_id, string $reply, bool $resolve): void
{
    ...
    if ($resolve && intval($ticket['ticket_status']) !== 4) {
        mysqli_query($mysqli, "UPDATE tickets SET ticket_status = 4, ticket_resolved_at = NOW()
