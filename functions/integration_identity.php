...
/**
 * Build a timestamp-insensitive identity projection for replay validation.
 *
 * Production inventory watermarks and synchronization timestamps are expected
 * to move on every poll. The durable asset set and source/object bindings are
 * not. Keeping that distinction explicit lets an operator compare a pre-replay
 * and post-replay export without claiming that a local fixture is production
 * burn-in evidence.
 */
function integrationIdentityReplayProjection(array $assets, array $mappings): array
{
    $asset_projection = [];
    foreach ($assets as $asset) {
        ...
        $active = !array_key_exists('active', $asset) || !empty($asset['active']);
        if (!$active) {
            continue;
        }
        $asset_id = max(0, intval($asset['asset_id'] ?? $asset['id'] ?? 0));
        $client_id = max(0, intval($asset['client_id'] ?? $asset['asset_client_id'] ?? 0));
        if ($asset_id < 1 || $client_id < 1) {
            continue;
        }
        $required_sources = array_values(array_unique(array_map(
            static fn ($source) => integrationIdentitySource($source),
            is_array($asset['required_sources'] ?? null)
                ? $asset['required_sources']
                : ['level', 'intune', 'entra', 'sentinelone']
        )));
        sort($required_sources, SORT_STRING);
        $source_scope_hashes = [];
        foreach ((array) ($asset['source_scopes'] ?? []) as $source => $scope_id) {
            $source = integrationIdentitySource($source);
            $scope_id = trim((string) $scope_id);
            if ($scope_id !== '') {
                $source_scope_hashes[$source] = hash('sha256', $scope_id);
            }
        }
        ksort($source_scope_hashes, SORT_STRING);
        $asset_projection[] = [
            'asset_id' => $asset_id,
            'client_id' => $client_id,
            'required_sources' => $required_sources,
            'source_scope_hashes' => $source_scope_hashes,
        ];
    }
    usort($asset_projection, static fn ($left, $right) => [
        $left['client_id'], $left['asset_id'],
    ] <=> [
        $right['client_id'], $right['asset_id'],
    ]);
    ...
    foreach ($mappings as $mapping) {
        ...
        $source = integrationIdentitySource(
            $mapping['source'] ?? $mapping['automation_mapping_source'] ?? ''
        );
        $external_id = integrationIdentityExternalId(
            $mapping['external_id'] ?? $mapping['automation_mapping_external_id'] ?? ''
        );
        $state = integrationIdentityState(
            $mapping['state'] ?? $mapping['automation_mapping_state'] ?? 'unresolved'
        );
        $mapping_projection[] = [
            'source' => $source,
            'external_id_hash' => hash('sha256', $source . "\0" . $external_id),
            'external_parent_id_hash' => hash('sha256', (string) (
                $mapping['external_parent_id']
                ?? $mapping['automation_mapping_external_parent_id']
                ?? ''
            )),
            'client_id' => max(0, intval(
                $mapping['client_id'] ?? $mapping['automation_mapping_client_id'] ?? 0
            )),
            'asset_id' => max(0, intval(
                $mapping['asset_id'] ?? $mapping['automation_mapping_asset_id'] ?? 0
            )),
            'state' => $state,
            'retired' => !empty(
                $mapping['deleted_at'] ?? $mapping['automation_mapping_deleted_at'] ?? null
            ) || $state === 'retired',
        ];
    }
    usort($mapping_projection, static fn ($left, $right) => [
        $left['source'], $left['external_id_hash'], $left['client_id'], $left['asset_id'],
    ] <=> [
        $right['source'], $right['external_id_hash'], $right['client_id'], $right['asset_id'],
    ]);

    $projection = ['assets' => $asset_projection, 'mappings' => $mapping_projection];
    return [
...
function integrationIdentityReplayInvariant(
    array $before_assets,
    array $before_mappings,
    array $after_assets,
    array $after_mappings
): array {
    $before = integrationIdentityReplayProjection($before_assets, $before_mappings);
    $after = integrationIdentityReplayProjection($after_assets, $after_mappings);
    $issues = [];
    if ($before['projection']['assets'] !== $after['projection']['assets']) {
        $issues[] = ['code' => 'asset_projection_changed'];
    }
    if ($before['projection']['mappings'] !== $after['projection']['mappings']) {
        $issues[] = ['code' => 'mapping_projection_changed'];
    }
    return [
        'passed' => !$issues,
...
/**
 * Evaluate a point-in-time Goal 1 burn-in export without mutating it.
 *
 * Callers supply only active managed assets and may override required_sources
 * per asset for an explicitly approved platform policy. A passing synthetic
 * fixture proves the verifier; only a scheduled-cycle production export can
 * prove production coverage.
 */
function integrationIdentityCoverageBurnInAudit(array $assets, array $mappings): array
{
    $supported_sources = ['level', 'intune', 'entra', 'sentinelone'];
    $asset_index = [];
    $required_links = 0;
    $issues = [];

    foreach ($assets as $asset) {
        if (!is_array($asset) || (array_key_exists('active', $asset) && empty($asset['active']))) {
            continue;
        }
        $asset_id = max(0, intval($asset['asset_id'] ?? $asset['id'] ?? 0));
        $client_id = max(0, intval($asset['client_id'] ?? $asset['asset_client_id'] ?? 0));
        if ($asset_id < 1 || $client_id < 1) {
            $issues[] = ['code' => 'invalid_asset_identity', 'asset_id' => $asset_id];
            continue;
        }
        if (isset($asset_index[$asset_id])) {
            $issues[] = ['code' => 'duplicate_asset_id', 'asset_id' => $asset_id];
            continue;
        }
        $required_sources = is_array($asset['required_sources'] ?? null)
            ? $asset['required_sources']
            : $supported_sources;
        $required_sources = array_values(array_unique(array_map(
            static fn ($source) => integrationIdentitySource($source),
        ...
        if (!$required_sources) {
            $issues[] = [
                'code' => 'required_source_policy_empty',
                'asset_id' => $asset_id,
            ];
        }
        foreach ($required_sources as $required_source) {
            if (!in_array($required_source, $supported_sources, true)) {
                throw new InvalidArgumentException(
                    "Unsupported endpoint inventory source in burn-in policy: $required_source"
                );
            }
        }
        sort($required_sources, SORT_STRING);
        ...
        foreach ((array) ($asset['source_scopes'] ?? []) as $source => $scope_id) {
            $source = integrationIdentitySource($source);
            if (in_array($source, $supported_sources, true) && trim((string) $scope_id) !== '') {
                $source_scopes[$source] = integrationIdentityLimitText($scope_id, 255);
            }
        }
        foreach ($required_sources as $required_source) {
            if (!isset($source_scopes[$required_source])) {
                $issues[] = [
                    'code' => 'required_source_scope_missing',
                    'asset_id' => $asset_id,
        ...
        $required_links += count($required_sources);
    }

    $durable_identity_rows = [];
    $queue_counts = array_fill_keys(['unresolved', 'suggested', 'conflicting', 'stale'], 0);
    $dispositioned = 0;
    foreach ($mappings as $mapping) {
        if (!is_array($mapping)) {
            $issues[] = ['code' => 'invalid_mapping_row'];
            continue;
        }
        try {
            $source = integrationIdentitySource(
            ...
            $issues[] = ['code' => 'invalid_mapping_identity'];
            continue;
        }
        if (!in_array($source, $supported_sources, true)) {
            continue;
        }
        $mapping_id = max(0, intval(
            $mapping['mapping_id'] ?? $mapping['automation_mapping_id'] ?? 0
        ));
        $asset_id = max(0, intval(
            $mapping['asset_id'] ?? $mapping['automation_mapping_asset_id'] ?? 0
        ));
        ...
        $identity_hash = hash('sha256', $source . "\0" . $external_id);
        $identity_key = $source . ':' . $identity_hash;
        if (!$deleted && !in_array($state, ['ignored', 'retired'], true)) {
            $durable_identity_rows[$identity_key][] = $mapping_id;
        }
        if ($deleted || in_array($state, ['ignored', 'retired'], true)) {
            $dispositioned++;
            continue;
        }
        if (isset($queue_counts[$state])) {
            $queue_counts[$state]++;
            $issues[] = [
                'code' => 'review_queue_not_clear',
                'source' => $source,
                'state' => $state,
                'mapping_id' => $mapping_id,
            ];
        }
        if (!in_array($state, ['automatic', 'confirmed'], true)) {
            continue;
        }
        if (!isset($asset_index[$asset_id])) {
            $issues[] = [
        ...
        $expected_scope = (string) ($asset['source_scopes'][$source] ?? '');
        if ($expected_scope !== '' && ($parent_id === '' || !hash_equals($expected_scope, $parent_id))) {
            $issues[] = [
        ...
        $asset_index[$asset_id]['trusted_mappings'][$source][] = $mapping_id;
    }

    foreach ($durable_identity_rows as $mapping_ids) {
        if (count($mapping_ids) > 1) {
            sort($mapping_ids, SORT_NUMERIC);
            $issues[] = [
                'code' => 'duplicate_durable_identity',
                'mapping_ids' => $mapping_ids,
            ];
        }
    }
    foreach ($asset_index as $asset) {
        foreach ($asset['required_sources'] as $source) {
            $mapping_ids = $asset['trusted_mappings'][$source] ?? [];
            if (count($mapping_ids) === 1) {
                continue;
            }
            sort($mapping_ids, SORT_NUMERIC);
            $issues[] = [
                'code' => count($mapping_ids) === 0
                    ? 'required_source_missing'
                    : 'required_source_duplicate',
    ...
    usort($issues, static fn ($left, $right) => strcmp(
        json_encode($left, JSON_UNESCAPED_SLASHES),
        json_encode($right, JSON_UNESCAPED_SLASHES)
    ));

    $summary = [
        'assets_checked' => count($asset_index),
        'required_links' => $required_links,
        'mappings_checked' => count($mappings),
        'dispositioned_mappings' => $dispositioned,
