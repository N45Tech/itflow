...
function printUsage($stream = STDOUT) {
    fwrite($stream, "Usage: php update_cli.php [--update_db | --bridge_n45_migrations]\n\n");
    ...
    fwrite($stream, "  --bridge_n45_migrations\n");
    fwrite($stream, "                Maintenance-only, one-time validation and bridge for an install\n");
    fwrite($stream, "                whose legacy N45 migrations used the upstream version marker.\n");
}
...
if (!$database_phase_only && count($arguments) > 0) {
    ...
    } elseif (count($arguments) === 1 && $arguments[0] === '--bridge_n45_migrations') {

        $database_phase_only = true;
        $bridge_only_requested = true;
...
// The bridge is deliberately separate from a normal update. It proves the
// legacy N45 schema/data fingerprints before restoring the upstream marker.
if ($bridge_only_requested) {
    try {
...
// Establish the fork-owned ledger before the upstream runner can advance into
// version numbers that an older N45 release used for different migrations.
try {
