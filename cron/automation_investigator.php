...
$summary = automationInvestigationRun(1);
if ($summary['status'] === 'unconfigured') {
    echo "Automation investigator: no dedicated AI model is configured; nothing queued.\n";
    return;
}

$details = "queued {$summary['queued']}; completed {$summary['completed']}; "
    . "failed {$summary['failed']}; dead {$summary['dead']}; skipped {$summary['skipped']}";
if ($summary['failed'] > 0 || $summary['dead'] > 0) {
    logApp('Automation Investigator', 'warning', "Read-only investigation run: $details");
}

echo "Automation investigator: $details.\n";
