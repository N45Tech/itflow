                ...
                <?php if ($level_asset_link) {
                    ...
                    <div class="card card-dark">
                        ...
                        <div class="card-body">
                            ...
                            <?php if ($level_device_role) { ?><div class="mt-2 text-capitalize"><i class="fas fa-fw fa-server text-secondary me-2"></i><?= $level_device_role ?></div><?php } ?>
                            <?php if ($level_last_logged_in_user) { ?><div class="mt-2"><i class="fas fa-fw fa-user-clock text-secondary me-2"></i><?= $level_last_logged_in_user ?></div><?php } ?>
                            <?php if ($level_cpu_cores || $level_total_memory) { ?>
                                <div class="mt-2"><i class="fas fa-fw fa-microchip text-secondary me-2"></i><?= $level_cpu_cores ? "$level_cpu_cores cores" : '' ?><?= $level_cpu_cores && $level_total_memory ? ' / ' : '' ?><?= $level_total_memory ? escapeHtml(formatBytes($level_total_memory, 1)) . ' RAM' : '' ?></div>
                            <?php } ?>
                            <?php if ($level_network_addresses) { ?><div class="mt-2"><i class="fas fa-fw fa-network-wired text-secondary me-2"></i><?= escapeHtml(implode(', ', $level_network_addresses)) ?></div><?php } ?>
                            <?php if ($level_network_mac) { ?><div class="mt-2"><i class="fas fa-fw fa-ethernet text-secondary me-2"></i><span class="font-monospace"><?= $level_network_mac ?></span></div><?php } ?>
                            <?php if ($level_device_security_score !== null) { ?>
                                <div class="mt-2"><i class="fas fa-fw fa-shield-alt text-secondary me-2"></i>Security score <?= intval($level_device_security_score) ?></div>
                            <?php } ?>
                            <?php if ($level_device_last_seen) { ?>
                                <div class="mt-2" title="<?= $level_device_last_seen ?>"><i class="fas fa-fw fa-eye text-secondary me-2"></i>Seen <?= escapeHtml(timeAgo($level_device_last_seen)) ?></div>
                            <?php } ?>
                            <div class="mt-2" title="<?= $level_device_last_synced ?>"><i class="fas fa-fw fa-sync-alt text-secondary me-2"></i>Synced <?= escapeHtml(timeAgo($level_device_last_synced)) ?></div>
                            <div class="mt-3 pt-2 border-top">
                                <a href="https://app.level.io/devices" target="_blank" rel="noopener noreferrer">Open Level devices <i class="fas fa-external-link-alt ms-1"></i></a>
                            </div>
                        </div>
                    </div>
                <?php } ?>
            ...
            <div class="col-md-9">
                ...
                <div class="card card-dark mb-3">
                    <div class="card-header py-2">
                        <h3 class="card-title mt-1"><i class="fa fa-fw fa-ethernet me-2"></i>Network Interfaces</h3>
                        <div class="card-tools">
                            <div class="btn-group">
