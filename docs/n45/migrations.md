...
The immutable `n45-0008` migration creates that historical four-column snapshot key before `n45-0012` widens it. Its `runner_fingerprint` therefore verifies the exact immediate post-migration shape, while its normal `fingerprint` remains the strict final six-column contract used by fresh-install validation. The legacy bridge selects the historical contract only through marker `2.7.8`; marker `2.7.9` and later must already match the final shape. This narrow runner contract preserves per-migration verification without weakening the completed-release schema contract.
