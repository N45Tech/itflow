...
The manifest reserves the next four feature IDs and all post-integration migrations so concurrent branches cannot collide or leave fork migrations in upstream's namespace:
...
| — | `n45-0019-ticket-approval-gates` | Runbooks |
| — | `n45-0020-specific-client-approvers` | Runbooks |
...
The immutable `n45-0008` migration creates that historical four-column snapshot key before `n45-0012` widens it. Its `runner_fingerprint` therefore supplies the exact immediate post-migration shape, while its normal `fingerprint` remains the strict final six-column contract used by fresh-install validation. The runner accepts the final contract first for an idempotent update over an already-final `db.sql`; otherwise it requires the exact runner contract until `n45-0012` applies. The legacy bridge selects the historical contract only through marker `2.7.8`; marker `2.7.9` and later must already match the final shape. This narrow fallback preserves per-migration verification without weakening the completed-release schema contract.
...
| `n45-0019-ticket-approval-gates` | — | Runbooks | No | Disable ticket approval creation, preserve approval evidence, and restore the pre-upgrade database snapshot before reverting application code. |
| `n45-0020-specific-client-approvers` | — | Runbooks | No | Disable specific client-contact routing, preserve approval evidence, and restore the pre-upgrade database snapshot before reverting application code. |
...
| `n45-0025-inbound-mail-receipts` | — | Runbooks | No | Preserve inbound mail receipts and source files; restore matching database and application snapshots before removing receipt tables. |
