...
function starterContentTicketTemplates() {
    ...
    $ticket_templates = [
        [
            ...
            'runbook_key' => 'new-user',
            'runbook_type' => 'standard',
            'publish_runbook' => true,
            'tasks' => [
                [
                    'key' => 'NUS-010',
                    'name' => 'NUS-010 Validate request authority and immutable scope',
                    'instructions' => 'Confirm the portal request is approved by a different eligible manager, the employee identity is unambiguous, and the start date, manager, role, location and requested outcome match the immutable submission. Stop for a new decision if scope has expanded.',
                    'estimate' => 20,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 2,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the requester, independent approver, employee identity, start time and approved scope without credentials.',
                    'depends_on' => [],
                ],
                [
                    'key' => 'NUS-020',
                    'name' => 'NUS-020 Define least-privilege access and license plan',
                    'instructions' => 'Translate the approved role into named groups, applications, mailbox resources, licenses and exclusions. Compare with an approved peer or role baseline, identify privileged or temporary access, and separate any device work.',
                    'estimate' => 25,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 4,
                    'evidence_type' => 'file',
                    'evidence_prompt' => 'Attach a redacted access and licensing plan with its role baseline and exclusions.',
                    'depends_on' => ['NUS-010'],
                ],
                [
                    'key' => 'NUS-030',
                    'name' => 'NUS-030 Approve provisioning and validation plan',
                    'instructions' => 'Obtain technical client approval for the exact least-privilege plan, provisioning time, temporary-access expiry, validation owner and rollback or disable path. Request approval again if any material term changes.',
                    'estimate' => 15,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 8,
                    ...
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the approved provisioning, validation and rollback plan.',
                    'depends_on' => ['NUS-020'],
                ],
                [
                    'key' => 'NUS-040',
                    'name' => 'NUS-040 Create the identity and secure initial access',
                    'instructions' => 'Create the directory identity with an attributable administrator, collision-safe username and approved start state. Use an approved one-time access method, require password change where applicable and never copy credentials into ITFlow.',
                    'estimate' => 25,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 12,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the durable identity, administrative actor, creation time and secure handoff method without secret values.',
                    'depends_on' => ['NUS-030'],
                ],
                [
                    'key' => 'NUS-050',
                    'name' => 'NUS-050 Apply approved licenses and access',
                    'instructions' => 'Assign only the approved licenses, groups, mailbox resources and line-of-business access. Confirm privileged and shared access remains excluded unless explicitly listed, and record any unavailable entitlement as an exception instead of substituting access.',
                    'estimate' => 40,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 16,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the applied licenses and access, excluded privileges and unresolved entitlement exceptions.',
                    'depends_on' => ['NUS-040'],
                ],
                [
                    'key' => 'NUS-060',
                    'name' => 'NUS-060 Link the approved device deployment',
                    'instructions' => 'When a managed device is part of the request, link the existing device-deployment ticket or create the separately scoped work from its published runbook. Do not hide purchasing, migration or old-device disposition inside this account task.',
                    'estimate' => 15,
                    'condition_type' => 'manual_confirm',
                    'condition_value' => 'The approved request includes a new or reassigned device',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 16,
                    'evidence_type' => 'url',
                    'evidence_prompt' => 'Link the device-deployment ticket or record why device work is not applicable.',
                    'depends_on' => ['NUS-030'],
                ],
                [
                    'key' => 'NUS-070',
                    'name' => 'NUS-070 Enroll MFA and validate the user outcome',
                    'instructions' => 'With the intended user or authorized validator, enroll approved authentication methods and verify sign-in, mail, file access and required applications. Test that unapproved privileged access is absent and disable or roll back the identity if validation materially fails.',
                    'estimate' => 30,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 24,
                    'initial_state' => 'Waiting',
                    'evidence_type' => 'any',
                    'evidence_prompt' => 'Attach or record MFA enrollment and positive and negative access-validation results.',
                    'depends_on' => ['NUS-050', 'NUS-060'],
                ],
                [
                    'key' => 'NUS-080',
                    'name' => 'NUS-080 Update records and send secure closeout',
                    'instructions' => 'Update the contact, licenses, authorized access, linked device and any expiry or review dates in ITFlow. Send the requester a completion and exception summary without passwords, recovery codes or sensitive access detail.',
                    'estimate' => 20,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 28,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the updated systems, completion message and unresolved exceptions or follow-up tickets.',
                    'depends_on' => ['NUS-070'],
                ],
            ],
        ],
        [
            ...
            'runbook_key' => 'termination',
            'runbook_type' => 'offboarding',
            'publish_runbook' => true,
            'tasks' => [
                [
                    'key' => 'TRM-010',
                    'name' => 'TRM-010 Validate authority, identity and effective time',
                    'instructions' => 'Confirm the immutable termination request was approved by a different eligible manager, identify the departing person without ambiguity, and verify the exact disable time, timezone, manager and authorized decision maker. Stop if timing or identity differs.',
                    'estimate' => 20,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 2,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the requester, independent approver, target identity and effective time without sensitive HR detail.',
                    'depends_on' => [],
                ],
                [
                    'key' => 'TRM-020',
                    'name' => 'TRM-020 Capture access, retention and asset baseline',
                    'instructions' => 'Inventory directory roles, groups, sessions, applications, shared credentials, mailbox and data ownership, licenses and assigned assets. Record legal-hold, retention, delegation and device-disposition requirements before access is changed.',
                    'estimate' => 35,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 4,
                    'evidence_type' => 'file',
                    'evidence_prompt' => 'Attach a redacted access, data, license and asset baseline plus the recorded retention decision.',
                    'depends_on' => ['TRM-010'],
                ],
                [
                    'key' => 'TRM-030',
                    'name' => 'TRM-030 Approve revocation, handoff and recovery plan',
                    'instructions' => 'Obtain technical client approval for the ordered disable and session-revocation actions, shared-secret rotations, data owner and retention treatment, communications, device handling, success checks and recovery path. Scope changes require another approval.',
                    'estimate' => 20,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 8,
                    ...
                    'approval_type' => 'technical',
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the approved revocation, handoff, device and recovery plan.',
                    'depends_on' => ['TRM-020'],
                ],
                [
                    'key' => 'TRM-040',
                    'name' => 'TRM-040 Block sign-in and revoke active access',
                    'instructions' => 'At the approved effective time, block sign-in, reset the account to an unrevealed random value, revoke active sessions and refresh tokens, and remove registered authentication methods as approved. Record timestamps and failures immediately.',
                    'estimate' => 25,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 12,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the systems, administrative actor, revocation timestamps and any failed control without secret values.',
                    'depends_on' => ['TRM-030'],
                ],
                [
                    'key' => 'TRM-050',
                    'name' => 'TRM-050 Remove privilege and rotate exposed shared access',
                    'instructions' => 'Remove directory and application roles, groups, delegations and shared access named in the plan. Rotate approved shared credentials or access paths the user knew, without storing replacement secrets in task evidence.',
                    'estimate' => 35,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 16,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record removed access and rotated systems using identifiers only, never replacement credentials.',
                    'depends_on' => ['TRM-040'],
                ],
                [
                    'key' => 'TRM-060',
                    'name' => 'TRM-060 Preserve and hand off business data',
                    'instructions' => 'Apply the approved mailbox conversion, delegation, forwarding, OneDrive, SharePoint and application-data handoff while preserving legal hold and retention. Do not delete the source identity or data merely to reclaim a license.',
                    'estimate' => 45,
                    'condition_type' => 'manual_confirm',
                    'condition_value' => 'The approved plan requires retained or reassigned business data',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 24,
                    'evidence_type' => 'any',
                    'evidence_prompt' => 'Attach or record the retained sources, approved data owner, handoff result and retention boundary.',
                    'depends_on' => ['TRM-040'],
                ],
                [
                    'key' => 'TRM-070',
                    'name' => 'TRM-070 Recover and quarantine assigned assets',
                    'instructions' => 'Coordinate return of devices, keys and access cards. Quarantine or reassign managed devices according to the approved disposition; never wipe, dispose of or transfer an asset without its separately recorded authorization.',
                    'estimate' => 30,
                    'condition_type' => 'manual_confirm',
                    'condition_value' => 'The departing user has assigned physical or managed assets',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 32,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record each asset, custody state and separately approved disposition or linked follow-up ticket.',
                    'depends_on' => ['TRM-030'],
                ],
                [
                    'key' => 'TRM-080',
                    'name' => 'TRM-080 Verify revocation and recover eligible licenses',
                    'instructions' => 'Test that interactive and application access is denied, privileged grants and active sessions are absent, approved delegation works and monitoring shows no unexpected use. Recover licenses only after retention and handoff conditions are satisfied.',
                    'estimate' => 30,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 36,
                    'evidence_type' => 'any',
                    'evidence_prompt' => 'Attach or record negative access tests, delegation checks, monitoring result and license decisions.',
                    'depends_on' => ['TRM-050', 'TRM-060', 'TRM-070'],
                ],
                [
                    'key' => 'TRM-090',
                    'name' => 'TRM-090 Update records and issue closeout',
                    'instructions' => 'Update contact status, identities, licenses, assets, access documentation and billing records while retaining required audit evidence. Send the authorized requester a completion, exception and follow-up summary without sensitive HR or credential data.',
                    'estimate' => 20,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 40,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record updated records, closeout recipient and every unresolved exception or follow-up ticket.',
                    'depends_on' => ['TRM-080'],
                ],
            ],
        ],
        ...
        [
            ...
            'runbook_key' => 'new-device',
            'runbook_type' => 'standard',
            'publish_runbook' => true,
            'tasks' => [
                [
                    'key' => 'DEV-010',
                    'name' => 'DEV-010 Validate request, entitlement and purchasing authority',
                    'instructions' => 'Confirm the portal request is approved by a different eligible manager, identify the assigned user and needed-by date, and verify the agreement, quote or purchase order covers the requested device and services. Stop rather than silently substituting billable scope.',
                    'estimate' => 20,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 4,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the requester, independent approver, entitlement or purchasing reference, user and required date.',
                    'depends_on' => [],
                ],
                [
                    'key' => 'DEV-020',
                    'name' => 'DEV-020 Confirm specification and management baseline',
                    'instructions' => 'Confirm hardware, warranty, operating system, user role, required applications, directory join, management, endpoint protection, backup, encryption, local-admin and network requirements. Identify exclusions and the old-device disposition.',
                    'estimate' => 30,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 8,
                    'evidence_type' => 'file',
                    'evidence_prompt' => 'Attach a redacted build sheet with required controls, applications, exclusions and old-device plan.',
                    'depends_on' => ['DEV-010'],
                ],
                [
                    'key' => 'DEV-030',
                    'name' => 'DEV-030 Approve build, migration and handoff plan',
                    'instructions' => 'Obtain technical client approval for the exact build baseline, assigned identity, migration source, validation owner, handoff method and old-device treatment. Any added application, privilege, destructive wipe or disposal requires separately recorded approval.',
                    'estimate' => 20,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 12,
                    ...
                    'approval_type' => 'technical',
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the approved build, migration, validation, handoff and old-device plan.',
                    'depends_on' => ['DEV-020'],
                ],
                [
                    'key' => 'DEV-040',
                    'name' => 'DEV-040 Inventory and patch the device',
                    'instructions' => 'Receive and identify the physical device, create or reconcile its ITFlow asset using verified serial identity, record warranty and ownership, update firmware, operating system and drivers, and capture the completed base-build state.',
                    'estimate' => 90,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 24,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the asset, serial, warranty, ownership and patched build versions.',
                    'depends_on' => ['DEV-030'],
                ],
                [
                    'key' => 'DEV-050',
                    'name' => 'DEV-050 Enroll identity, management and security controls',
                    'instructions' => 'Join the approved directory, verify device ownership, configure encryption and escrow its recovery key in the approved system, and install and verify only in-scope management, monitoring, endpoint protection and backup controls.',
                    'estimate' => 60,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 32,
                    'evidence_type' => 'any',
                    'evidence_prompt' => 'Attach or record directory ownership, encryption escrow and each in-scope management/security control health result.',
                    'depends_on' => ['DEV-040'],
                ],
                [
                    'key' => 'DEV-060',
                    'name' => 'DEV-060 Install approved applications and least privilege',
                    'instructions' => 'Install the approved standard and line-of-business applications, apply assigned-user access, configure required peripherals and network resources, and confirm the user has no unintended local administrator or application privilege.',
                    'estimate' => 60,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 40,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record installed applications, configured resources, assigned identity and negative privilege check.',
                    'depends_on' => ['DEV-050'],
                ],
                [
                    'key' => 'DEV-070',
                    'name' => 'DEV-070 Migrate user data from a recoverable source',
                    'instructions' => 'When migration is approved, verify a recoverable source copy before transferring the authorized profile and data. Preserve the source until acceptance and retention requirements pass; do not copy excluded or another user\'s data.',
                    'estimate' => 90,
                    'condition_type' => 'manual_confirm',
                    'condition_value' => 'The approved request includes profile or business-data migration',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 48,
                    'evidence_type' => 'file',
                    'evidence_prompt' => 'Attach a redacted source-recovery check and migration result, or record why migration is not applicable.',
                    'depends_on' => ['DEV-030', 'DEV-060'],
                ],
                [
                    'key' => 'DEV-080',
                    'name' => 'DEV-080 Validate controls and obtain user acceptance',
                    'instructions' => 'Verify sign-in, MFA, mail, file synchronization, required applications, printing, network access, encryption, management, protection, backup where in scope and absence of unintended privilege. Resolve failures or return to the approved recovery path before handoff.',
                    'estimate' => 40,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 56,
                    'initial_state' => 'Waiting',
                    'evidence_type' => 'any',
                    'evidence_prompt' => 'Attach or record technical checks and acceptance by the intended user or authorized validator.',
                    'depends_on' => ['DEV-050', 'DEV-060', 'DEV-070'],
                ],
                [
                    'key' => 'DEV-090',
                    'name' => 'DEV-090 Document handoff and route the old device',
                    'instructions' => 'Update the asset, assigned user, warranty, endpoint posture, applications and linked evidence in ITFlow; record custody and delivery; and route any old device to separately approved reuse, retention, return, wipe or disposal work.',
                    'estimate' => 25,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 60,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the asset updates, custody handoff, closeout communication and old-device follow-up reference.',
                    'depends_on' => ['DEV-080'],
                ],
            ],
        ],
        ...
        [
            ...
            'description' => 'Triage, contain, resolve and document a reported service or security incident',
            'subject' => 'Incident response - [Client] - [Summary]',
            'details' => '<p>A portal incident creates work immediately without a request-approval delay. Preserve the immutable report, establish impact and safety first, and require a separate recorded decision before a disruptive production change or external notification.</p>',
            'runbook_key' => 'incident-report',
            'runbook_type' => 'standard',
            'publish_runbook' => true,
            'tasks' => [
                [
                    'key' => 'INC-010',
                    'name' => 'INC-010 Acknowledge and preserve the reported facts',
                    'instructions' => 'Acknowledge the reporter, preserve the immutable portal responses, identify the affected service, device, users, start time and business impact, and establish a safe communication path. Do not delay initial response for catalog approval.',
                    'estimate' => 15,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 1,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record acknowledgement time, reporter, affected scope, observed symptoms and declared impact.',
                    'depends_on' => [],
                ],
                [
                    'key' => 'INC-020',
                    'name' => 'INC-020 Classify severity, security and escalation',
                    'instructions' => 'Triage current availability, spread, safety, privilege, data exposure and contractual urgency. Declare the operational severity, identify the technical and client decision makers, and invoke the security-incident path when compromise or data concern cannot be excluded.',
                    'estimate' => 20,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 2,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record severity, classification basis, decision makers, escalation path and next update time.',
                    'depends_on' => ['INC-010'],
                ],
                [
                    'key' => 'INC-030',
                    'name' => 'INC-030 Stabilize service or contain active risk',
                    'instructions' => 'When active impact or spread requires immediate action, use the least-destructive reversible containment available, preserve evidence before remediation where possible, log every action and stop if the response would exceed emergency authority.',
                    'estimate' => 30,
                    'condition_type' => 'manual_confirm',
                    'condition_value' => 'The incident has active impact, spread or safety risk requiring containment',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 3,
                    'evidence_type' => 'any',
                    'evidence_prompt' => 'Attach or record the pre-action state, containment action, authority, timestamps and resulting service/risk state.',
                    'depends_on' => ['INC-020'],
                ],
                [
                    'key' => 'INC-040',
                    'name' => 'INC-040 Diagnose cause and define recovery checks',
                    'instructions' => 'Build a testable cause hypothesis from monitoring, logs, recent changes and affected dependencies. Define the proposed correction, affected systems, success criteria, monitoring period, rollback trigger and evidence-preservation requirements.',
                    'estimate' => 45,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 6,
                    'evidence_type' => 'file',
                    'evidence_prompt' => 'Attach a redacted diagnostic timeline or export and record the proposed recovery and validation plan.',
                    'depends_on' => ['INC-020', 'INC-030'],
                ],
                [
                    'key' => 'INC-050',
                    'name' => 'INC-050 Approve disruptive remediation or notification',
                    'instructions' => 'If recovery changes production configuration, causes material interruption, destroys evidence or requires insurer, legal, regulatory or other external notification, obtain technical client approval for the exact action and rollback. Independent legal obligations are escalated, never improvised.',
                    'estimate' => 15,
                    'condition_type' => 'manual_confirm',
                    'condition_value' => 'The proposed response is disruptive, destructive, externally notifiable or materially changes production',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 8,
                    'initial_state' => 'Waiting',
                    'approval_scope' => 'client',
                    'approval_type' => 'technical',
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the exact approved action, approver, limits, notification authority, rollback trigger and expiry.',
                    'depends_on' => ['INC-040'],
                ],
                [
                    'key' => 'INC-060',
                    'name' => 'INC-060 Remediate and recover within the approved boundary',
                    'instructions' => 'Apply the least-privilege correction or restore from a known-good state, follow the approved sequence, preserve an attributable action timeline and execute rollback when its trigger is met. Create separately approved work for any expanded scope.',
                    'estimate' => 60,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 12,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record actions, actors, timestamps, deviations, recovery source and any rollback or follow-up work.',
                    'depends_on' => ['INC-040', 'INC-050'],
                ],
                [
                    'key' => 'INC-070',
                    'name' => 'INC-070 Validate service, security and monitoring',
                    'instructions' => 'Test the original failure and affected dependencies, confirm intended service is restored, verify no unintended access or security regression, compare monitoring with the baseline and observe for recurrence for the declared interval.',
                    'estimate' => 30,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 16,
                    'initial_state' => 'Waiting',
                    'evidence_type' => 'any',
                    'evidence_prompt' => 'Attach or record functional, negative-security and monitoring results plus client acceptance or remaining impact.',
                    'depends_on' => ['INC-060'],
                ],
                [
                    'key' => 'INC-080',
                    'name' => 'INC-080 Document cause, outcome and follow-up',
                    'instructions' => 'Update affected service, asset, configuration and incident records; record cause and confidence without unsupported certainty; send a concise client outcome; and create owned remediation or post-incident review tickets for unresolved risk.',
                    'estimate' => 25,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 20,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the documented cause, client closeout, residual risk, owner and linked follow-up ticket IDs.',
                    'depends_on' => ['INC-070'],
                ],
            ],
        ],
        ...
        [
            ...
            'description' => 'Bring a signed client into the Managed Care operating model with one clear owner and a practical handoff',
            'subject' => 'Managed Care onboarding - [Client]',
            'details' => '<p>Use this checklist to move from signed agreement to a supported client. Keep source documents attached to the client, record exceptions as notes, and create separate follow-up tickets for work that should outlive onboarding.</p>',
            ...
            'tasks' => [
                [
                    'key' => 'ONB-010',
                    'name' => 'ONB-010 Confirm agreement, scope and onboarding owner',
                    'instructions' => 'Attach the signed agreement, confirm the service start date and scope, and name the N45 owner and authorized client decision maker.',
                    'estimate' => 30,
                    'condition_type' => 'always',
                    ...
                    'evidence_type' => 'file',
                    'evidence_prompt' => 'Attach the signed agreement or approved agreement record.',
                    'depends_on' => [],
                ],
                [
                    'key' => 'ONB-020',
                    'name' => 'ONB-020 Hold kickoff and capture key contacts',
                    'instructions' => 'Confirm primary, technical, billing and emergency contacts, escalation paths, locations, maintenance constraints and the target handoff date.',
                    'estimate' => 60,
                    'condition_type' => 'always',
                    'owner_type' => 'project_manager',
                    'due_offset_hours' => 24,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record attendees, contacts, decisions and the target handoff date.',
                    'depends_on' => ['ONB-010'],
                ],
                [
                    'key' => 'ONB-030',
                    'name' => 'ONB-030 Document the current environment and access',
                    'instructions' => 'Capture users, devices, sites, networks, critical applications, vendors, administrative access and known risks at the level needed to support the client.',
                    'estimate' => 180,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 48,
                    'evidence_type' => 'file',
                    'evidence_prompt' => 'Attach the current-state discovery notes or inventory export.',
                    'depends_on' => ['ONB-020'],
                ],
                [
                    'key' => 'ONB-040',
                    'name' => 'ONB-040 Create core client records and attach documents',
                    'instructions' => 'Reconcile the client, locations, contacts, services, vendors and assets in ITFlow. Attach the documents technicians need for delivery and leave document-specific review to onboarding and future service reviews.',
                    'estimate' => 120,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 72,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the core records created, duplicates resolved and any missing source material.',
                    'depends_on' => ['ONB-030'],
                ],
                [
                    'key' => 'ONB-050',
                    'name' => 'ONB-050 Connect in-scope management, security and backup tools',
                    'instructions' => 'Deploy or connect only the tools included in the agreement, confirm each source maps to the correct client and record exclusions or unsupported systems.',
                    'estimate' => 180,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 96,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record connected tools, device counts, exclusions and unresolved mapping issues.',
                    'depends_on' => ['ONB-040'],
                ],
                [
                    'key' => 'ONB-060',
                    'name' => 'ONB-060 Validate the Microsoft 365 baseline when in scope',
                    'instructions' => 'Confirm delegated access, emergency accounts, licensing, identity protections, endpoint policy and mail security. Record exceptions and create separate remediation tickets where needed.',
                    'estimate' => 120,
                    ...
                    'evidence_type' => 'file',
                    'evidence_prompt' => 'Attach the baseline summary or record why Microsoft 365 is not in scope.',
                    'depends_on' => ['ONB-040'],
                ],
                [
                    'key' => 'ONB-070',
                    'name' => 'ONB-070 Test support, monitoring and recovery handoff',
                    'instructions' => 'Verify alert routing, remote support, portal access, backup visibility and one agreed recovery check. Confirm technicians can find the information needed to respond.',
                    'estimate' => 90,
                    'condition_type' => 'always',
                    'owner_type' => 'ticket_assignee',
                    'due_offset_hours' => 144,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record the tests performed, results and follow-up tickets.',
                    'depends_on' => ['ONB-050', 'ONB-060'],
                ],
                [
                    'key' => 'ONB-080',
                    'name' => 'ONB-080 Complete the handoff and schedule the first service review',
                    'instructions' => 'Review supported services and open risks with the client, confirm support channels, assign remaining work, and schedule the first client service review.',
                    'estimate' => 60,
                    'condition_type' => 'always',
                    'owner_type' => 'project_manager',
                    'due_offset_hours' => 168,
                    'evidence_type' => 'note',
                    'evidence_prompt' => 'Record acceptance, open work with owners, and the first review date.',
                    'depends_on' => ['ONB-070'],
                ],
            ],
        ],
        ...
        [
            ...
            'description' => 'Prepare and deliver a focused client service review',
            'subject' => 'Client service review - [Client] - [Period]',
            'details' => '<p>Use the service-review snapshot as the agenda. Focus on decisions and next actions, not raw ticket counts. Any action that survives the meeting should become an owned ticket or project task.</p>',
            'tasks' => [
                ['Prepare the service, ticket and SLA summary', 30],
                ['Review recurring issues, protection, backups and lifecycle risks', 45],
                ['Confirm key documents, contacts and upcoming renewals', 30],
                ['Hold the client review and record decisions', 60],
                ['Create follow-up work, assign owners and schedule the next review', 30],
            ],
        ],
        [
