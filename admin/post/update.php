...
if (isset($_GET['queue_update'])) {
    ...
    // Fail closed server-side as well as hiding the button. Updating application
    // files while the fork ledger is ambiguous can move the upstream marker past
    // migrations that have not been fingerprinted or recorded.
    try {
        $n45_status = n45MigrationStatus($mysqli);
        if (($n45_status['state'] ?? '') === 'bridge_required' || !empty($n45_status['errors'])) {
            flashAlert('Update queueing is blocked until the N45 migration bridge or integrity condition is resolved.', 'error');
            redirect();
        }
    } catch (Throwable $e) {
        flashAlert('Update queueing is blocked because the N45 migration status could not be verified.', 'error');
        redirect();
    }
