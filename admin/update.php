...
$n45_queue_blocked = $n45_bridge_required || !empty($n45_integrity_errors);
...
<div class="card card-dark">
    ...
    <div class="card-body">
        ...
        <?php if (!empty($update_checked_at) && !$app_update_available && !$database_work_available) { ?>
        ...
        <?php } else { ?>
            ...
            <?php if ($app_update_available || $db_update_available || $n45_update_available || empty($update_checked_at)) { ?>
                <div class="mb-4">
                    ...
                    <?php if ($n45_update_available) { ?>
                        <p class="mb-2">
                            <strong>N45 database stream:</strong>
                            <?= $n45_pending ?> checksum-verified migration<?= $n45_pending === 1 ? '' : 's' ?> pending
                            (<?= $n45_applied ?> of <?= $n45_total ?> applied).
                        </p>
                    <?php } ?>

                    <?php if ($update_queue_available && !$n45_queue_blocked) { ?>
                    ...
                    <p class="text-muted mt-2 mb-0">
                        <small>
                            <?php if ($n45_queue_blocked) { ?>
                                <strong>Queueing is blocked by the N45 migration guard below.</strong>
                                Resolve the bridge or integrity condition before updating application files or either
                                migration stream.
                            <?php } elseif ($update_queue_available) { ?>
            ...
            <?php if (!empty($n45_integrity_errors)) { ?>
                <div class="alert alert-danger">
                    <h5><i class="fas fa-fw fa-shield-alt me-2"></i>N45 migration integrity check failed</h5>
                    The updater is blocked until these ledger or inventory errors are resolved:
                    <ul class="mb-0 mt-2">
            ...
            <?php } elseif ($n45_bridge_required) { ?>
                <div class="alert alert-warning">
                    <h5><i class="fas fa-fw fa-exclamation-triangle me-2"></i>One-time N45 migration bridge required</h5>
                    This install still uses legacy fork marker
                    <strong><?= escapeHtml($n45_status['upstream_marker']) ?></strong>. In a maintenance window, verify
                    a restorable database backup, then run the bridge. It fingerprints the current schema and data,
                    records checksum ledger entries, and restores only the upstream marker to
                    <strong><?= escapeHtml($n45_status['upstream_marker_base'] ?? '') ?></strong>.
                    <div class="mt-3">
                        <a class="btn btn-warning confirm-link" href="post.php?bridge_n45_migrations&csrf_token=<?= $_SESSION['csrf_token'] ?>">
                            <i class="fas fa-fw fa-link me-2"></i>Bridge Legacy N45 Migrations
                        </a>
                    </div>
                </div>
            <?php } ?>
